...
export class MarathonProjectFakeInterceptor implements HttpInterceptor {
    ...
    setHealthy(id){
        let groups: any[] = JSON.parse(localStorage.getItem('groups')) || [];
    ...
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
        ...
        return of(null).pipe(mergeMap(() => {
            ...
            if (request.url.endsWith('/groups') && request.method === 'GET') {
                // check for fake auth token in header and return users if valid, this security is implemented server side in a real grouplication
            ...
            if (request.url.match(/\/groups\/\d+$/) && request.method === 'GET') {
                // check for fake auth token in header and return user if valid, this security is implemented server side in a real grouplication
                if (request.headers.get('Authorization') === 'Bearer fake-jwt-token') {
                    // find user by id in users array
                    let urlParts = request.url.split('/');
                    let id = parseInt(urlParts[urlParts.length - 1]);
                    let matchedUsers = groups.filter(user => { return user.id === id; });
            ...
            if (request.url.endsWith('/groups') && request.method === 'POST') {
                ...
                // validation
                let duplicategroup = groups.filter(group => { return group.id === newgroup.id; }).length;
                if (duplicategroup) {
                ...
                let pointer = groups.push(newgroup);
                localStorage.setItem('groups', JSON.stringify(groups));
            ...
            if (request.url.match(/\/groups\//) && request.method === 'DELETE') {
                ...
                // if (request.headers.get('Authorization') === 'Bearer fake-jwt-token') {
                    // find user by id in users array
                    let urlParts = request.url.split('/groups/');
                    // let id = parseInt(urlParts[urlParts.length - 1]);
